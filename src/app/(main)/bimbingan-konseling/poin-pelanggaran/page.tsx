...
export default function PelanggaranPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await bkService.findAllPelanggaran();
      // Asumsikan responsenya berisi array data
      setData(res || []);
    } catch (error: any) {
      console.error(error);
      message.error('Gagal mengambil data poin pelanggaran');
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      ...getColumnSearchProps('jenisPelanggaran', 'Cari Pelanggaran'),
    },
    {
      title: 'Poin',
      dataIndex: 'poin',
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <Space size="middle">
          <Tooltip title="Detail Riwayat Pelanggaran Siswa">
            <ButtonToolbar 
              message="" 
              icon={<EyeOutlined style={{ color: '#1677ff' }} />} 
              className="bg-blue-50 text-blue-600 hover:bg-blue-100" 
              onClick={() => router.push(`/bimbingan-konseling/poin-pelanggaran/${record.siswaId || record.id}`)}
            />
          </Tooltip>
        </Space>
      ),
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 bg-slate-50 p-4 pt-2 relative">
      <div className="mb-2 text-gray-500 text-sm">Bimbingan Konseling / Poin Pelanggaran</div>
      ...
      <div className="data-induk-table-wrapper bg-white px-4 pb-4 pt-1 mt-1 rounded-lg shadow-sm border border-gray-100">
        <Table 
          ...
          pagination={{ 
            ...
            showTotal: (total, range) => `Menampilkan ${range[0]}-${range[1]} dari ${total} pelanggaran`
          }}
